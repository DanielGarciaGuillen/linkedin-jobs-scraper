...
const linksSelector =
  ".jobs-search__results-list li a.result-card__full-card-link";
...
const dateSelector = ".job-result-card__listdate";
/*  "#main-content > div > section > ul > li.result-card.job-result-card.result-card--with-hover-state.job-card__contents--active > div.result-card__contents.job-result-card__contents > div > time"; */
/* 
.job-card-search__time-badge */
...
const _loadMoreJobs = async (
  ...
  while (!loaded) {
    if (!clicked) {
      clicked = await page.evaluate((seeMoreJobsSelector) => {
...
function LinkedinScraper(options) {
  ...
  const _run = async (
    ...
    for (const tuple of queriesXlocations) {
      ...
      await page.evaluate(
        () =>
          (document.querySelector(`form#JOBS input[name="keywords"]`).value =
            "")
      );
      await page.evaluate(
        () =>
          (document.querySelector(`form#JOBS input[name="location"]`).value =
            "")
      );
          ...
          try {
            [[jobId, jobLink], loadJobDetailsResponse] = await Promise.all([
              page.evaluate(
                (linksSelector, jobIndex) => {
                  const linkElem = document.querySelectorAll(linksSelector)[
                    jobIndex
                  ];
                  linkElem.click();
                  return [
            ...
            if (descriptionProcessor) {
              jobDescription = await page.evaluate(
                `(${descriptionProcessor.toString()})();`
              );
            } else {
              jobDescription = await page.evaluate((descriptionSelector) => {
  ...
  this.run = async (
    queries,
    locations,
    { paginationMax, descriptionProcessor } = {
      paginationMax: 10,
    ...
    try {
      ...
      await _run(queries, locations, paginationMax, descriptionProcessor);
    } catch (err) {
      logger.error(err);
